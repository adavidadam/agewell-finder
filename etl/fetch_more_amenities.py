"""
Add leisure centres, gyms, community centres, and bus stops to our database.
"""
import osmnx as ox
import sqlite3
import time
...
# Add new columns
try:
    cursor.execute('ALTER TABLE amenities ADD COLUMN leisure_centres INTEGER DEFAULT 0')
    cursor.execute('ALTER TABLE amenities ADD COLUMN gyms INTEGER DEFAULT 0')
    cursor.execute('ALTER TABLE amenities ADD COLUMN community_centres INTEGER DEFAULT 0')
    cursor.execute('ALTER TABLE amenities ADD COLUMN bus_stops INTEGER DEFAULT 0')
except:
    pass  # Columns might already exist

print("🏋️ Fetching leisure centres, gyms, community centres, and bus stops...\n")
...
for i, (town, (lat, lon)) in enumerate(TOWNS.items(), 1):
    ...
    try:
        ...
        # Community centres
        community = ox.features_from_point(
            (lat, lon),
        ...
        cursor.execute('''
            ...
            WHERE town = ?
        ''', (leisure_count, gym_count, community_count, bus_count, town))
        
        conn.commit()
        
        print(f"✅ LC:{leisure_count} G:{gym_count} CC:{community_count} Bus:{bus_count}")
...
print("\n✨ Complete! Updated data/agewell.db")
